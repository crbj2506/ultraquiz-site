                    ...
                    <tbody>
                        @foreach ($sugestoes as $key => $s)
                            <tr>
                                ...
                                <td class="text-center py-0" scope="row"><a href="{{ route('sugestaopormim.mostrar',['sugestao' => $s])}}" class="btn btn-sm btn-outline-primary py-0">Ver</a></td>
                                <td class="text-center py-0" scope="row">
                                    @if($s->verificacoes->count() === 0)
                                        <a href="{{ route('sugestaopormim.editar',['sugestao' => $s])}}" class="btn btn-sm btn-outline-warning py-0">Editar</a></td>
                                    @endif
                            </tr>
                        @endforeach
                    </tbody>
